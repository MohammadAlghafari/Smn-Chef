...
Future<Stream<Notification>> getNotifications() async {
  ...
  try {
    if (_user.id == null)  return new Stream.value(Notification.fromJSON({}));;
...
Future<Notification> markAsReadNotifications(Notification notification) async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url_customer')}notifications/${notification.id}?$_apiToken';
  final client = new http.Client();
...
Future<Notification> removeNotification(Notification cart) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
      '${GlobalConfiguration().getValue('api_base_url_customer')}notifications/${cart.id}?$_apiToken';
  final client = new http.Client();
