...
void getRestaurantLocation() async {
    try {
      ...
      setState(() {
        cameraPosition = CameraPosition(
          target: LatLng(double.parse(restaurant.latitude), double.parse(restaurant.longitude)),
          zoom: 12.4746,
        );
      });
        Helper.getRestaurantPositionMarker(double.parse(restaurant.latitude) , double.parse(restaurant.longitude)).then((marker) {
          setState(() {
            allMarkers.add(marker);
          });
        });
